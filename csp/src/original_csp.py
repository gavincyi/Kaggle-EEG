...
for subject in subjects:
    ...
    ################ READ DATA ################################################
    fnames =  ['../data/train/subj%d_series%d_data.csv' % (subject, index) for index in range(1, 4)]
    ...
    # read test data
    fnames =  ['../data/train/subj%d_series%d_data.csv' % (subject, index) for index in range(7, 9)]
    raw = concatenate_raws([creat_mne_raw_object(fname, read_events=True) for fname in fnames])
    raw._data[picks] = np.array([lfilter(b,a,raw._data[i]) for i in picks])
